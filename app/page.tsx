...
export default function Home() {
  ...
  if (!worldData) return (
    <div className="h-[100dvh] flex flex-col items-center justify-center bg-[#e5e5e5] text-stone-500 gap-4">
      ...
      <div className="font-mono text-xs tracking-widest">INITIALIZING PIXEL WORLD...</div>
    </div>
  );
  ...
  const ControlPanel = () => (
    <div className="flex flex-col h-full bg-[#f8f8f8]">
      <div className="p-4 bg-white border-b border-gray-200">
        <h2 className="text-[10px] font-bold text-gray-400 uppercase tracking-widest mb-3">Storage</h2>
        <div className="grid grid-cols-2 gap-2.5">
           <div className="flex justify-between text-xs p-2 bg-stone-50 rounded-md border border-stone-100"><span className="text-stone-500">木材</span> <span className="font-bold text-stone-800">{globalResources.wood}</span></div>
           <div className="flex justify-between text-xs p-2 bg-stone-50 rounded-md border border-stone-100"><span className="text-stone-500">石料</span> <span className="font-bold text-stone-800">{globalResources.stone}</span></div>
           <div className="flex justify-between text-xs p-2 bg-stone-50 rounded-md border border-stone-100"><span className="text-stone-500">食物</span> <span className="font-bold text-stone-800">{globalResources.food}</span></div>
           <div className="flex justify-between text-xs p-2 bg-stone-50 rounded-md border border-stone-100"><span className="text-stone-500">药品</span> <span className="font-bold text-stone-800">{globalResources.medicine}</span></div>
        </div>
      </div>
      <div className="flex-1 p-4 overflow-y-auto bg-[#fafaf9]">
        <h2 className="text-[10px] font-bold text-gray-400 uppercase tracking-widest mb-3">Blueprints</h2>
        <div className="space-y-2.5">
          {BUILD_OPTIONS.map(opt => (
            <button key={opt.type} onClick={() => handleBuild(opt.type)} className="w-full bg-white p-2.5 rounded-lg border border-gray-200 shadow-sm active:scale-95 transition-all text-left flex justify-between items-center group hover:border-blue-400">
              <div>
                <div className="font-bold text-gray-700 text-sm group-hover:text-blue-600">{opt.name}</div>
                <div className="text-[10px] text-gray-400 font-mono mt-0.5">{opt.cost}</div>
              </div>
              <Construction size={16} className="text-gray-300 group-hover:text-blue-500" />
            </button>
          ))}
        </div>
      </div>
      <div className="p-4 border-t border-gray-200 bg-gray-50 mt-auto">
        <button onClick={handleReset} className="w-full py-2.5 bg-white text-red-500 rounded border border-red-100 font-bold hover:bg-red-50 active:bg-red-100 text-xs tracking-wider flex items-center justify-center gap-2 transition-colors">
    ...
    <div className="flex flex-col h-full bg-[#f0f0f0]">
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-3">
         {logs.slice().reverse().map((log: string, i: number) => (
           <div key={i} className={`p-4 rounded-xl border leading-relaxed ${i===0 ? 'bg-white shadow-md border-blue-200' : 'bg-[#f7f7f5] border-transparent text-gray-500'}`}>
    ...
    <div className="flex flex-col h-full bg-white">
      ...
      <div className="flex-1 overflow-y-auto p-3 bg-gray-50 space-y-2.5">
         {rightTab === 'ai' ? agents.map((agent: Agent) => (
           <div key={agent.id} className="bg-white p-2.5 rounded-xl border border-gray-200 shadow-sm flex gap-2.5 group hover:border-blue-300 transition-colors">
              ...
              <div className="flex-1 min-w-0">
                 ...
                 <div className="h-1.5 bg-gray-100 rounded-full overflow-hidden w-full mb-1">
           ...
           <div key={npc.id} className="bg-white p-2.5 rounded-lg border border-gray-200 flex justify-between items-center shadow-sm">
              <div>
  ...
  return (
    <div className="flex flex-col h-[100dvh] bg-[#e5e5e5] text-stone-800 font-sans overflow-hidden">
      ...
      <div className="hidden md:flex flex-1 overflow-hidden">
        {/* 左侧控制栏：稍微调窄 */}
        <aside className="w-60 border-r border-gray-300 z-10 shadow-lg"><ControlPanel /></aside>
        
        {/* 中间主区域 */}
        <main className="flex-1 flex flex-col min-w-0 bg-[#f0f0f0] z-0">
           {/* 地图区域：增加高度，利用宽屏优势 */}
           <div className="h-[60%] min-h-[500px] border-b border-gray-300 relative bg-white shadow-sm z-10">
              <PixelMap worldData={worldData} />
           </div>
           {/* 日志区域 */}
        ...
        {/* 右侧人员栏：稍微调窄 */}
        <aside className="w-64 border-l border-gray-300 z-10 shadow-lg"><RosterPanel /></aside>
      </div>

      {/* 移动端布局 */}
      <div className="md:hidden flex-1 overflow-hidden relative bg-[#f0f0f0]">
        {mobileView === 'logs' && <LogPanel />}
        {mobileView === 'map' && <PixelMap worldData={worldData} />}
        {mobileView === 'control' && <ControlPanel />}
        {mobileView === 'roster' && <RosterPanel />}
      </div>

      {/* 移动端底部导航 */}
      <nav className="md:hidden h-14 bg-white border-t border-gray-200 flex justify-around items-center shrink-0 z-50 pb-safe shadow-[0_-4px_15px_-3px_rgba(0,0,0,0.1)] relative">
        ...
        <button onClick={() => setMobileView('logs')} className={`flex flex-col items-center justify-center w-14 h-full transition-colors ${mobileView==='logs'?'text-blue-600':'text-gray-400'}`}>
          <FileText size={18} />
          <span className="text-[9px] font-bold mt-0.5">LOGS</span>
        </button>
        <button onClick={() => setMobileView('map')} className={`flex flex-col items-center justify-center w-14 h-full transition-colors ${mobileView==='map'?'text-blue-600':'text-gray-400'}`}>
          <MapIcon size={18} />
          <span className="text-[9px] font-bold mt-0.5">MAP</span>
        </button>
        <button onClick={() => setMobileView('control')} className={`flex flex-col items-center justify-center w-14 h-full transition-colors ${mobileView==='control'?'text-blue-600':'text-gray-400'}`}>
          <Construction size={18} />
          <span className="text-[9px] font-bold mt-0.5">BUILD</span>
        </button>
        <button onClick={() => setMobileView('roster')} className={`flex flex-col items-center justify-center w-14 h-full transition-colors ${mobileView==='roster'?'text-blue-600':'text-gray-400'}`}>
          <Users size={18} />
          <span className="text-[9px] font-bold mt-0.5">TEAM</span>
        </button>
      </nav>
