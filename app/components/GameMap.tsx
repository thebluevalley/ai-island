...
import { motion, AnimatePresence } from 'framer-motion'; // 如果没有安装，标准CSS也能做，这里用CSS替代以防依赖报错
...
export default function GameMap({ worldData, onSelectAgent }: { worldData: any, onSelectAgent: (agent: any) => void }) {
  const containerRef = useRef<HTMLDivElement>(null);
  const bgCanvasRef = useRef<HTMLCanvasElement>(null);
  
  const [fontSize, setFontSize] = useState(8);
  const [cellSize, setCellSize] = useState({ w: 0, h: 0 }); // 用于定位 HTML 元素
  const { agents } = worldData || { agents: [] };

  // --- Map Generation (Static Layer) ---
  const { grid } = useMemo(() => {
            ...
            offsets.forEach(off => {
                ...
                if(px>=0 && px<MAP_COLS && py>=0 && py<MAP_ROWS) {
                    ...
                    if(!grid[idx].isBldg && grid[idx].char !== CHARS.ROAD_MAIN) {
                        setCell(px, py, CHARS.PATH, COLORS.FG_PATH, COLORS.BG_GRASS, true);
                    }
                }
            });
        };
    ...
    const drawBuilding = (x: number, y: number, w: number, h: number, type: 'RES' | 'COM' | 'CIV' | 'CAFE', facing: string = 'DOWN') => {
        ...
        if (facing === 'UP') { d1y = y; } else if (facing === 'DOWN') { d1y = y + h - 1; }
        else if (facing === 'LEFT') { d1x = x; d1y = y + Math.floor(h/2); } else if (facing === 'RIGHT') { d1x = x + w - 1; d1y = y + Math.floor(h/2); }
    ...
    for (let bx=0; bx<GRID_COLS; bx++) {
        const sx = startX + bx * BLOCK_W + MAIN_ROAD_W; const sw = BLOCK_W - MAIN_ROAD_W;
    ...
    for (let by=0; by<GRID_ROWS; by++) {
        for (let bx=0; bx<GRID_COLS; bx++) {
            ...
            let count = 12; if (rand < 0.2) count = 10; else if (rand < 0.4) count = 11;
            ...
            for(let iy=sy; iy<sy+sh; iy++) for(let ix=sx; ix<sx+sw; ix++) {
                ...
                if (grid[idx].char === CHARS.GRASS && !grid[idx].isRoad && !grid[idx].isBldg) {
                    if (random(ix, iy) > 0.85) setCell(ix, iy, CHARS.GRASS, COLORS.FG_POCKET, COLORS.BG_GRASS);
                }
            }
        }
    }
  ...
  useEffect(() => {
    ...
    const resizeObserver = new ResizeObserver(entries => {
        for (let entry of entries) {
            ...
            // Calculate pixel size for HTML overlay
            const dpr = window.devicePixelRatio || 1;
            setCellSize({ w: (width / MAP_COLS), h: (height / MAP_ROWS) });
        }
    });
  ...
  // --- HTML Overlay Layer (Agents & Bubbles) ---
  return (
    <div ref={containerRef} className="w-full h-full bg-[#23242a] flex items-center justify-center overflow-hidden p-4 relative">
      ...
      {/* HTML Overlay for Interactive Agents */}
      <div className="absolute inset-0 m-auto" style={{
          width: '100%', height: '100%', 
          position: 'absolute', top: 0, left: 0
      }}>
        {cellSize.w > 0 && agents.map((agent: any) => {
            const x = agent.x * cellSize.w;
            const y = agent.y * cellSize.h;
            ...
            return (
                <div 
                    ...
                    className="absolute cursor-pointer transition-all duration-300 ease-linear hover:z-50 group"
                    style={{
                        left: x, top: y,
                        width: cellSize.w * 1.5, height: cellSize.h * 1.5,
                ...
                >
                    ...
                    {/* Speech Bubble (Show on hover or if speaking) */}
                    {(agent.speech || agent.isHovered) && (
                        <div className="absolute left-1/2 bottom-full mb-1 -translate-x-1/2 w-32 bg-white text-black p-2 rounded shadow-lg text-[10px] z-20 pointer-events-none opacity-0 group-hover:opacity-100 transition-opacity">
                            {agent.speech || "..."}
